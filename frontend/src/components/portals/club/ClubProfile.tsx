...
import { 
  Building2, MapPin, Phone, Globe, ShieldCheck, Check, X, Edit, 
  Camera, ImageIcon, User, Users, Activity, ClipboardList, Briefcase
} from "lucide-react";
...
const CLUB_STARS = [
  [12,8],[38,15],[60,25],[20,33],[75,44],[44,54],[8,65],[65,74],[85,20],[30,30],[52,40],[70,50],[25,60],[48,70],[78,10],
];
...
export const ClubProfile = ({ demoClub, ridersCount, horsesCount, regCount }: ClubProfileProps) => {
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState({ ...demoClub });
    ...
    <div className="animate-fade-in pb-12">
      ...
      <div className="-mx-4 sm:-mx-6 -mt-3 sm:-mt-6 relative">
        <div className="relative h-72 sm:h-96 lg:h-[450px] overflow-hidden">
          {bannerImage ? (
            <img src={bannerImage} alt="Club banner" className="absolute inset-0 w-full h-full object-cover" />
          ) : (
            <>
              {/* Complex Layered Background */}
              <div className="absolute inset-0" style={{ background: "linear-gradient(155deg, #0f0608 0%, #18090d 18%, #220d12 35%, #1c0a0d 55%, #1a0709 78%, #100506 100%)" }} />
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                 <span className="px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase bg-primary/10 border border-primary/30 text-primary backdrop-blur-xl shadow-xl flex items-center gap-2">
                    <ShieldCheck className="w-3 h-3 animate-pulse" /> Authorized EFI Unit
                 </span>
              ...
              <div className="flex items-center gap-6 text-sm text-white/50 tracking-wide font-medium uppercase font-mono">
                ...
                <span className="flex items-center gap-2 space-x-2">
                   <Users className="w-4 h-4 opacity-50 font-black" /> 
                   <span>{ridersCount} Athletes</span>
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="relative px-6 sm:px-12 -mt-16 sm:-mt-24 flex items-end justify-between z-30">
          <div className="relative group/avatar cursor-pointer">
            <div className="w-32 h-32 sm:w-44 sm:h-44 rounded-[2.5rem] overflow-hidden bg-surface-container border-[6px] border-background shadow-2xl flex items-center justify-center transition-all duration-500 group-hover/avatar:rotate-2 group-hover/avatar:scale-105">
                 {profileImage 
                    ...
                    : <div className="w-full h-full bg-gradient-to-br from-primary/20 to-secondary/10 flex items-center justify-center text-6xl shadow-inner">🏛️</div>
                 }
            </div>
          ...
          <div className="pb-4 sm:pb-8 flex gap-3">
             <button onClick={() => setEditing(!editing)} 
               ...
               {editing ? <><X className="w-4 h-4" /> Cancel Calibration</> : <><Edit className="w-4 h-4" /> Recalibrate Club Info</>}
             </button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 lg:mt-12 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 lg:space-y-8">
           <div className="bento-card p-6 sm:p-8 relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-8 rotate-12 opacity-[0.03] select-none pointer-events-none transition-transform group-hover:scale-110 duration-700">
                <Building2 className="w-64 h-64 font-black" />
              </div>
              <h3 className="text-sm font-black uppercase tracking-widest text-on-surface mb-6 opacity-70">Club Charter</h3>
              <p className="text-lg font-medium text-on-surface-variant leading-relaxed font-serif italic border-l-4 border-primary/30 pl-6 mb-8">
                Operating as a premier EFI-Authorized equestrian center. Dedicated to fostering elite competitive talent and maintaining operational excellence in the <span className="text-on-surface font-black">{form.city}</span> circuit.
              </p>
              
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                 {[
                   { label: "Total Riders", value: ridersCount, sub: "EFI Registered Athletes" },
                   { label: "Club Horses", value: horsesCount, sub: "Authorized Stable" },
                   { label: "Season Entries", value: regCount, sub: "Circuit Performance" },
                   { label: "Revenue Rate", value: "85%", sub: "Collection Efficiency" },
                 ].map((stat, i) => (
           ...
           {/* Club Details Form (Conditionally Interactive) */}
           <div className="bento-card p-6 sm:p-8">
              <h3 className="text-sm font-black uppercase tracking-widest text-on-surface mb-8 opacity-70">Core Operational Data</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                <Field label="Protocol Email" required><Input disabled={!editing} value={form.email} onChange={set("email")} /></Field>
                <Field label="Communication Line" required><Input disabled={!editing} value={form.phone} onChange={set("phone")} /></Field>
                <Field label="Official Domain"><Input disabled={!editing} value={form.website || ""} onChange={set("website")} placeholder="https://..." /></Field>
                <Field label="Founder / Admin"><Input disabled={!editing} value={form.presidentName} onChange={set("presidentName")} /></Field>
                <Field label="EFI Federation ID" required><Input disabled={!editing} value={form.efiClubId} onChange={set("efiClubId")} /></Field>
                <Field label="Taxation ID (GST)"><Input disabled={!editing} value={form.gstNumber || ""} onChange={set("gstNumber")} placeholder="Optional / Pending" /></Field>
              </div>
        ...
        <div className="space-y-6">
           <div className="bento-card p-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-[0.03] rotate-12"><Briefcase className="w-24 h-24" /></div>
              <h3 className="text-xs font-black uppercase tracking-widest text-on-surface mb-6 opacity-70">Management Comms</h3>
              <div className="space-y-5">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Primary Registry Line</p>
                       <p className="text-sm font-bold">{form.phone}</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-secondary/10 flex items-center justify-center text-secondary shadow-inner"><Globe className="w-5 h-5" /></div>
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Operational Website</p>
                       <p className="text-sm font-bold truncate max-w-[150px]">{form.website || "Not Set"}</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="bento-card p-6 relative overflow-hidden">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-on-surface mb-6 opacity-70">Club Locale</h3>
              <div className="space-y-5">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Unit Headquarters</p>
